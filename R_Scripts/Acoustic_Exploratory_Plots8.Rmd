...
fit.hfaci1 <- lmer(ACI_HF ~ Snaps*t12 + (1|Site) + Year, data = Snap.HF)
summary(fit.hfaci1)
...
```{r model snaps, echo = TRUE}
fit.tg <- lmer(SPL_HF ~ Snaps*tg + (1|Site) + Year, data = Snap.HFC)
fit.dn <- lmer(SPL_HF ~ Snaps*dn + (1|Site) + Year, data = Snap.HFC)
fit.ns <- lmer(SPL_HF ~ Snaps*ns + (1|Site) + Year, data = Snap.HFC)
fit.t12 <- lmer(SPL_HF ~Snaps*t12 + (1|Site) + Year, data = Snap.HFC)

#models that have a 2 way interaction and time seperately as a factor


AICc(fit.tg, fit.dn, fit.ns, fit.t12)
...
#Plots to help me understand

##Sites and SPL Plot 

```{r help plot 1, echo = FALSE}

site.plot <- ggplot(AC.DF1, aes(Tot_Knocks, SPL_Midrange, color = Site)) + geom_point() + geom_smooth(method = "lm", se=F) + theme_classic() + labs(y = "Low Frequency SPL", x = "Number of Knocks")
site.plot <- ggplot(AC.DF1, aes(Site, SPL_Midrange)) + geom_boxplot()
site.plot
