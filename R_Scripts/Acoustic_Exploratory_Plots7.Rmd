...
Snap.HF$Snaps <- center_scale(Snap.HF$Snaps)
...
#fit.hfaci <- glm(ACI_HF ~(Snaps*t12*Site) + Year, data = Snap.HF, family = "Gamma")
#summary(fit.a)
#stepAIC(fit.a)
...
fit.tg <- lm(SPL_HF ~ Snaps*tg*Site + Year, data = Snap.HF)
fit.dn <- lm(SPL_HF ~ Snaps*dn*Site + Year, data = Snap.HF)
fit.ns <- lm(SPL_HF ~ Snaps*ns*Site + Year, data = Snap.HF)
fit.t12 <- lm(SPL_HF ~Snaps*t12*Site + Year, data = Snap.HF)

#models that have a 2 way interaction and time seperately as a factor
fit.t12t <- lm(SPL_HF ~ Snaps*Site + t12 + Year, data = Snap.HF)
fit.tgt <- lm(SPL_HF ~ Snaps*Site + tg + Year, data = Snap.HF)
fit.dnt <- lm(SPL_HF ~ Snaps*Site + dn + Year, data = Snap.HF)
fit.nst <- lm(SPL_HF ~ Snaps*Site + ns + Year, data = Snap.HF)
...
snap.model <- lm(SPL_HF ~ Snaps + t12 + Site + Snaps:t12 + Snaps:Site + t12:Site, data = Snap.HF)

Anova(snap.model, type = 3)
...
IP1 <- interaction.plot(Snap.HF$t12, Snap.HF$Snaps, Snap.HF$SPL_HF)

IP2 <- interaction.plot(Snap.HF$Site, Snap.HF$Snaps, Snap.HF$SPL_HF)

IP3 <- interaction.plot(Snap.HF$t12, Snap.HF$Site, Snap.HF$SPL_HF)
...
##High Frequency SPL Model - fixing distribution

###trying different data transformations

Because my QQ plot above is so skewed I am going to try to transform the data to fit a normal distribution

First, checking my distribution of all of my data
...
Second, backtransformed (Because SPL is inherently log transformed)

```{r transform, echo = TRUE}

Snap.HF$btdata <- exp(Snap.HF$SPL_HF)

snap.modelbt <- lm(btdata ~ Snaps + t12 + Site + Snaps:t12 + Snaps:Site + t12:Site, data = Snap.HF)
...
```{r reciprocal transform, echo = TRUE}
Snap.HF$recdata <- 1/(Snap.HF$SPL_HF)

snap.modelrec <- lm(recdata ~ Snaps + t12 + Site + Snaps:t12 + Snaps:Site + t12:Site, data = Snap.HF)
...
Determining which is the best way to group the snaps by time * with a gamma distribution
...
```{r model snaps2, echo = TRUE}
fit.tg <- glm(SPL_HF ~ Snaps*tg*Site + Year, data = Snap.HF, family = "Gamma")
fit.dn <- glm(SPL_HF ~ Snaps*dn*Site + Year, data = Snap.HF, family = "Gamma")
fit.ns <- glm(SPL_HF ~ Snaps*ns*Site + Year, data = Snap.HF, family = "Gamma")
fit.t12 <- glm(SPL_HF ~Snaps*t12*Site + Year, data = Snap.HF, family = "Gamma")

#models that have a 2 way interaction and time seperately as a factor
fit.t12t <- glm(SPL_HF ~ Snaps*Site + t12 + Year, data = Snap.HF, family = "Gamma")
fit.tgt <- glm(SPL_HF ~ Snaps*Site + tg + Year, data = Snap.HF, family = "Gamma")
fit.dnt <- glm(SPL_HF ~ Snaps*Site + dn + Year, data = Snap.HF, family = "Gamma")
fit.nst <- glm(SPL_HF ~ Snaps*Site + ns + Year, data = Snap.HF, family = "Gamma")

AICc(fit.tg, fit.dn, fit.ns, fit.t12, fit.t12t, fit.tgt, fit.dnt, fit.nst)

```

t12 is still the best split, like before

Gamma distribution, using a glm

```{r hf gamma, echo = TRUE}
snap.model2 <- glm(SPL_HF ~ Snaps*t12*Site + Year, data = Snap.HF, family = "Gamma")

stepAIC(snap.model2)

```

Therefore, my most parsimonious model is

```{r hf gamma best, echo = TRUE}
snap.model2b <- glm(SPL_HF ~ Snaps + t12 + Site + Snaps:t12 + Snaps:Site + t12:Site, data = Snap.HF, family = "Gamma")

summary(snap.model2b)
```
