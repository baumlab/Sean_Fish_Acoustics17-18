...
ki.5abo <- ki.5 %>% group_by(Year, KI.Date, Time, Site, Observer, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))
ki.5ab <- ki.5 %>% group_by(Year, KI.Date, Time, Site, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))

ki.8abo <- ki.8 %>% group_by(Year, KI.Date, Time, Site, Observer, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))
ki.8ab <- ki.8 %>% group_by(Year, KI.Date, Time, Site, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))

ki.32abo <- ki.32 %>% group_by(Year, KI.Date, Time, Site, Observer, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))
ki.32ab <- ki.32 %>% group_by(Year, KI.Date, Time, Site, Species, Family, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))

ki.35abo <- ki.35 %>% group_by(Year, KI.Date, Time, Site, Observer, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))
ki.35ab <- ki.35 %>% group_by(Year, KI.Date, Time, Site, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))
 
ki.40abo <- ki.40 %>% group_by(Year, KI.Date, Time, Site, Observer, Species, CoarseFG, FineFG) %>% summarise(Abundance = sum(Number))
...
## Site abundance 2018
```{r, echo = FALSE}
d1 <- ggplot(ki18.5, aes(Family, Abundance)) + geom_bar(aes(fill = KI.Date), stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle=90, vjust=0, size = 9))
d1

d2 <- ggplot(ki18.8, aes(Family, Abundance)) + geom_bar(aes(fill = KI.Date), stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle=90, vjust=0, size = 9))
d2

d3 <- ggplot(ki18.32, aes(Family, Abundance)) + geom_bar(aes(fill = KI.Date), stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle=90, vjust=0, size = 9))
d3

d4 <- ggplot(ki18.35, aes(Family, Abundance)) + geom_bar(aes(fill = KI.Date), stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle=90, vjust=0, size = 9))
d4

d5 <- ggplot(ki18.40, aes(Family, Abundance)) + geom_bar(aes(fill = KI.Date), stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle=90, vjust=0, size = 9))
d5
