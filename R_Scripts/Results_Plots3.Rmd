...
algae.3 <- lm(SPL_HF ~ Snaps*dn + Site/Year, data = Snap.HFa)
summary(algae.3)
```

When I replace macroalgae with site/year the first thing you see is that site/year describes changes in HF SPL much better than algae. We now explain 64.5 % of variation in HF SPL using this model. You see that all sites were louder in 2018 (site 40 omitted due to lack of algae data in 2018). We also see all sites except site 40 are signficantly different from site 32. Additionally, the effect size of snaps is larger than the algae model at 0.015 and at night event stronger ( ~ 0.021). At night here the reefs are on average 5 dB quiter than during the day (this seems wrong based on the diurnal plots and I will need investigate).



###AIC selection
```{r algae SPL HF selection, echo = FALSE}
AICc(algae.2a, algae.2, algae.3)
...
algae.a3 <- lm(ACI_HF ~ Snaps*dn + Site/Year, data = Snap.HFa)
summary(algae.a3)
```

When I replace macroalgae with site and year as a nested fixed effect, sites are significantly different (evidence of ACI being able to determine site differences?? circumstantial evidence??) Night is not significantly different from day and snaps are still negatively associated with changes in ACI. ACI is only significantly different at site 35 between years. We also see that here 18.8% of the variation in HF ACI is explained by this model compared with only 10% explained in the macroalgae model.
...
Site and Year again is the better model here. 
...
regmodel <- lm(SPL_Midrange ~ Tot_Knocks + Hour + Tot_Knocks:Hour + Site/Year + Tot_Knocks:Hour, data = AC.DF1Coa)

summary(regmodel)
```

When I add in site and year as fixed effects here, the effect of total knocks shrinks to - 0.0207. But at 9 AM that effect is increased to ~ .028 So every 30 knocks results in increased SPL at 9 AM.  Interactions at other hours result in effect sizes that are nearly 0 at all other times.  9 AM is significantly 1.75 dB louder than 3 PM.  Additionally all sites are louder in 2018 than they were in 2017. Except site 40 which we don't have algae data for. Also, this model explains 75.5% of variation in MF SPL which is really cool!
...
So when I include macroalgae here, the only significant factors are the interaction between knocks and time at 9 AM and macroalgal cover. This means algal cover is obscuring significance in other variables. This model only explains 32.6% of the variation around MF SPL compared to 75.5% above. 
...
AICc(regmodela, regmodel)


```

Go figure, site/year is the best model here as well.
...
fit.mfaci7 <- lm(ACI_Midrange ~ Tot_Knocks*Hour + Num_Herbivory + Num_L_calls + Site/Year, data = AC.DF1Coa)
summary(fit.mfaci7)

```

Alright, so with this I see that the only significant variables are site and site:year variables. This means that all variation is just attributed to differences between sites and years (not actual sound production)... So what is driving changes in ACI??? Suprisingly, these variables account for 62.3% of variation in MF ACI
...
So now the only significant variable is algal cover which increases ACI by 2700 for each extra percent cover? This wouldn't seem to make sense as algae has been associated with dampening sounds... not making them more complex? Also this model now only explains about 11.7% of the variation in MF ACI.

```{r mf aic, echo = FALSE}

AICc(fit.mfacia, fit.mfaci7)


```

Yet again, site/year is the best model.
