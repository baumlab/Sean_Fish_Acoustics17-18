...
*Note* 2017 had a 10 minute duty cycle with 5 minutes on while 2018 had a 15 minute duty cycle with 5 minutes on, so the number of files averages differs between years
...
#Need to rename the columns in the dataframe

p1 <- ggplot(data =SPL_BB17.hr.long, aes(x=as.numeric(Hour), y=Broadband_SPL)) + geom_line(aes(color=as.character(Site))) + xlab("Hour") + ylab ("BB_SPL") + theme(legend.position = "bottom", legend.title = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank()) + scale_color_brewer(palette = "Set1")
