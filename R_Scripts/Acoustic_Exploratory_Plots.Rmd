...
### Broadband
```{r spl hf no av, echo = FALSE}
#renaming columns
names(SPL_BB17) <- c("Date", "Time", "s32", "s35", "s40", "s8", "s5")
#Making date a character
SPL_BB17$Date <- as.character(SPL_BB17$Date)

ggplot(data =SPL_BB17, aes(x=Time, y=s32)) + geom_line()
ggplot(data = SPL_BB17, aes(x = Time, y = s35)) + geom_line()
ggplot(data = SPL_BB17, aes(x = Time, y = s5)) + geom_line()
