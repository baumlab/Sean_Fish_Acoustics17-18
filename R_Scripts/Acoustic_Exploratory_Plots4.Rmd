...
#loading in zoomed spectrograms
spec20.3 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987850.180620030004.jpg")
spec20.9 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987850.180620090002.jpg")
spec20.21 <- image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987850.180620210002.jpg")
spec24.9 <- image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987850.180624090002.jpg")
spec25.9 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987850.180625090002.png.jpg")
spec26.9 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987850.180626090002.jpg")
spec27.9 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987850.180627090002.jpg")
spec12.9 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170712090002.jpg")
spec12.21 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170712210002.jpg")
spec13.3 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170713030002.jpg")
spec13.21 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170713210002.jpg")
spec19.3 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170719030002.jpg")
spec19.9 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170719090002.jpg")
spec21.9 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170721090002.jpg")

spec14.15 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170714150002.jpg")
spec12.15 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170712150002.jpg")
spec14.21 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170714210002.jpg")
spec21.3 <-image_read("../Sound_Photos/Small_Spectrogram_Photos/1677987881.170721030002.jpg")


#loading power data
load("../Raw_Data/KtotE.Rdata")
load("../Raw_Data/KtotP.Rdata")
load("../Raw_Data/MtotP.Rdata")
load("../Raw_Data/StotP.Rdata")
...
#prepping to plot distributions Ktot

#seperating into the high or low values
KElow <- filter(KtotE, st.id %in% low5$st.id)
KEhigh <- filter(KtotE, st.id %in% high5$st.id)

KPlow <- filter(KtotP, st.id %in% low5$st.id)
KPhigh <- filter(KtotP, st.id %in% high5$st.id)

#adding column for grouping
KElow$Group <- "Low"
KEhigh$Group <- "High"
...
#rebinding these together
KEtot <- rbind(KElow, KEhigh)
KPtot <- rbind(KPlow, KPhigh)


#Prepping to plot distributions Mtot

#seperating into the high or low values
MPlow <- filter(MtotP, st.id %in% low5$st.id)
MPhigh <- filter(MtotP, st.id %in% high5$st.id)
...
#rebinding
MPtot <- rbind(MPlow, MPhigh)

##Prepping to plot distributions Stot

#seperating into the high or low values
SPlow <- filter(StotP, st.id %in% low5$st.id)
SPhigh <- filter(StotP, st.id %in% high5$st.id)
...
#rebinding
SPtot <- rbind(SPlow, SPhigh)

#removing outliers
#low.SP <- SPtot[SPtot$AvPowerDiff <1, ]

# Remove anything with fewer than 2 observers 

#SPtot <- SPtot[!(SPtot$st.id %in% low.SP$st.id),]
#now rerunning without the 3 outliers

```

Grouped based on being either from the Top 10 SPL files (High) o the Bottom 10 SPL files (Low)

```{r power plots, echo = FALSE}
#ggplot(KEtot, aes(x = EnergyDiff)) +
#  geom_histogram(data=subset(KEtot, Group == "Low"), fill = "red", alpha = 0.2) + 
#  geom_histogram(data=subset(KEtot, Group == "High"), fill = "blue", alpha = 0.2)

ggplot(KPtot, aes(x = PowerDiff)) +
  geom_histogram(data=subset(KPtot, Group == "Low"), fill = "red", alpha = 0.2) +
  geom_histogram(data=subset(KPtot, Group == "High"), fill = "blue", alpha = 0.2)

ggplot(KPtot, aes(x = Group, y = PowerDiff)) + geom_boxplot()

```

Well, it sure doesn't look like the differences are stemming from the knocks themselves. Lets look at the frequency band as a whole.

Here are some zoomed in spectrograms.  I did this because I think background or "unidentifiable" noise is creating the differences we see.

My 5 highest Mid-SPL's at site 5 come from these 5 files they are all at 9 AM (interestingly, they are all from 2018)

```{r zoom spectro, echo = FALSE}
spechigh <- image_append(c(spec20.9, spec27.9, spec25.9, spec26.9, spec24.9))
spechigh <- image_resize(spechigh, "5000")
spechigh

```

My 5 lowest Mid-SPL's at site 5 comes from these 5 files, none are from 9 AM (they are all from 2017)

The times they come from are: 15, 3, 15, 3, 21

```{r zoom spectro2, echo = FALSE}
speclow <- image_append(c(spec14.15, spec19.3, spec12.15, spec21.3, spec14.21))
speclow <- image_resize(speclow, "5000")
speclow

```

*Mid Frequency Samples*

So for this next set of plots.  I took ~4 samples from each file that were 5 seconds long and extended from ~ 60 Hz to 1 kHz.  Samples were taken at :10, 1:10, 2:10, and 3:10 in each file.

```{r mid power plots, echo = FALSE}
#ggplot(MPtot, aes(x = PowerDiff)) + 
#  geom_histogram(data=subset(MPtot, Group == "Low"), fill = "red", alpha = 0.2) +
#  geom_histogram(data=subset(MPtot, Group == "High"), fill = "blue", alpha = 0.2)

ggplot(MPtot, aes(x = Group, y = PowerDiff)) + geom_boxplot() + stat_summary(fun.y = mean, color = "blue", geom = "point")
```

*100 - 600 Hz Samples*

I narrowed it down to the band of background noise I think is creating the difference. I took 4 10 second samples in each file.

```{r small power plots, echo = FALSE}

ggplot(SPtot, aes(x = Group, y = PowerDiff)) + geom_boxplot() + stat_summary(fun.y = mean, color = "blue", geom = "point") + ggtitle("Peak Power")
ggplot(SPtot, aes(x = Group, y = AvPowerDiff)) + geom_boxplot() + stat_summary(fun.y = mean, color = "blue", geom = "point") + ggtitle("Average Power")

APD.av <- aggregate(AvPowerDiff ~ Group, SPtot, mean)
APD.se <- aggregate(AvPowerDiff ~ Group, SPtot, function(x) sd(x)/sqrt(length(x)))
APD.av$SE <- APD.se$AvPowerDiff
APD.av$U.SE <- APD.av$AvPowerDiff + APD.av$SE
APD.av$L.SE <- APD.av$AvPowerDiff - APD.av$SE


APD.CI <- ggplot(APD.av, aes(Group, AvPowerDiff)) + 
  geom_errorbar(aes(ymin = L.SE, ymax = U.SE), color = "black", width = 0) +
  geom_point(aes(fill = Group), color = "black", pch = 21, size = 6) +
  labs(x = "Group", y = "AvPowerDiff", title = "Average Power")
APD.CI
```
