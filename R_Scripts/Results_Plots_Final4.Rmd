...
aci.hf5 <- lmer(ACI_HF ~1 + (1|site_year), data = Snap.HF)
summary(aci.hf5)
```
...
acihftbl <- AICc(aci.hf, aci.hf1a, aci.hf2, aci.hf4, aci.hf5)
...
acimftbl <-AICc(mf.aci1, mf.aci2a, mf.aci2b, mf.aci3, mf.aci4, fit.mfaci6, fit.mfaci7, mf.maxaci, mf.acil, mf.acih, mf.acilh, mf.acihh, inter.mfaci)
...
acimftbl.short <- subset(acimftbl, deltaAIC < 100)
...
r.squaredGLMM(fit.mfaci7)
r.squaredGLMM(mf.acihh)
r.squaredGLMM(mf.acilh)
```
...
#subsetting only one site and one year
Snap.HFT5 <- Snap.HFT[Snap.HFT$Site == 5,]
Snap.HFT518 <- Snap.HFT5[Snap.HFT5$Year == 2018,]

#fixing time

Snap.HFT518$Time2 <- sprintf("%04d", Snap.HFT518$Time2)
Snap.HFT518$Time2 <- as.character(Snap.HFT518$Time2)
Snap.HFT518$Time3 <- strftime(Snap.HFT518$datetime, format = "%H:%M")
...
ACI_MF_tot5 <- ACI_MF_tot[ACI_MF_tot$Site == 5,]
ACI_MF_tot518 <- ACI_MF_tot5[ACI_MF_tot5$Year == 2018,]

#recreating time column in Snap.HF
Snap.HF$Time <- paste(Snap.HF$Hour, Snap.HF$Minutes, sep = ":")

cbp2 <- c("#E69F00", "#000000", "#56B4E9", "#009E73",
...
## Knock Intensity Plot ####
```{r knock intensity plot, echo = FALSE}
sel.tbl$Hour <- as.factor(sel.tbl$Hour)
knocks.bp <- ggplot(sel.tbl, aes(Hour, RL)) + geom_boxplot() + theme_classic() + ylab("Sound Pressure Level (dB)") + 
...
ggsave(file = "../Figures/knocksbp.eps", width = 11, height = 7, knocks.bp)

#confidence interval plot

#taking average and calculating confidence intervals for sites
RL.av <- aggregate(RL ~ Hour, sel.tbl, mean)
RL.se <- aggregate(RL ~ Hour, sel.tbl, function(x) sd(x)/sqrt(length(x)))
RL.av$SE <- RL.se$RL
RL.av$U.SE <- RL.av$RL + RL.av$SE
RL.av$L.SE <- RL.av$RL - RL.av$SE



#knock.ci <- ggplot(RL.av, aes(Hour, RL)) + 
  geom_errorbar(aes(ymin = L.SE, ymax = U.SE), color = "black", width = 0.15) +
  geom_point(aes(fill = Hour), color = "black", pch = 21, size = 6) +
  labs(x = "Hour", y = "Knock Volume (dB)")
knock.ci

#ggsave(file = "../Figures/knockci.eps", width = 11, height = 7, knock.ci)
...
SP2.hf <- ggplot(Snap.HF5, aes(Snaps, SPL_HF, color = t12)) + geom_point(size = 0.5) + labs(color = "Day/Night") + scale_color_discrete(labels = c("Night", "Day")) + theme_classic() + labs(y = "High Frequency SPL")+ geom_abline(intercept = 87.37, slope = 0.021, col = "firebrick2") + geom_abline(intercept = 92.94, slope = 0.016, col = "cyan3")

SP2.hf <- SP2.hf + annotate("text", x =1200, y = 118, label = "italic(R) ^ 2 == 0.69", parse = TRUE)
SP2.hf

#ggsave(file = "../Figures/HFSPL.pdf", width = 11, height = 7, SP2.hf)

#ACI and snaps

#removing outliers from ACI plot to make sure that the negative trend remains
low.snap <- Snap.HF5[Snap.HF5$Snaps <1200, ]
# Remove anything with fewer than 1200 snaps
Snap.HF5s <- Snap.HF5[!(Snap.HF5$Snaps %in% low.snap$Snaps),]

#ACI and snaps
AC2.hf <- ggplot(Snap.HF5s, aes(Snaps, ACI_HF)) + geom_point(size = 1.2) + theme_classic() + labs(y = "High Frequency ACI") + 
  geom_abline(intercept = 72219.98 , slope = -5.4515, col = "blue")
AC2.hf

#ggsave(file = "../Figures/AC2.hf.pdf", width = 11, height = 7, AC2.hf)
...
SP.mf <- ggplot(AC.DF1, aes(Tot_Knocks, SPL_Midrange,color = Hour, shape = Hour)) + 
  ...
  scale_color_manual(values=cbp2) + xlim(0, 350) +
  geom_abline(intercept = 104.7271, slope = 0.002328, col = "#009E73") + 
  geom_abline(intercept = 106.6348, slope = 0.030271, col = "#000000") + 
  geom_abline(intercept = 104.909369, slope = -0.017715, col = "#56B4E9") + 
  geom_abline(intercept = 105.9475, slope = 0.000474, col = "#E69F00")
SP.mf

#ggsave(file = "../Figures/LFSPL.pdf", width = 11, height = 7, SP.mf)

#geom_smooth(aes(color = Hour, fill = Hour),method = "lm", se = T)
#+ scale_fill_manual(values= cbp2)

AC.mf <- ggplot(AC.DF1, aes(Tot_Knocks, ACI_Midrange, color = Hour, shape = Hour)) + geom_point() +
  theme_classic() + labs (y = "Low Frequency ACI", x = "Number of Knocks") + scale_color_manual(values=cbp2) + geom_smooth(method = lm)

AC.mf


#ggsave(file = "../Figures/LFACI.pdf", width = 11, height = 7, AC.mf)


#lfsplest <- plot_model(reducemf4)

#ggsave(file = "../Figures/LFSPLEstimates.pdf", width = 11, height = 7, lfsplest)
```
...
This shows that the pattern exists at pretty much all sites except for 5.


```{r mf ACI plot, echo = FALSE}

#lfaciest <- plot_model(mf.maxaci)
#lfaciest

#ggsave(file = "../Figures/LFACIEstimates.pdf", width = 11, height = 7, lfaciest)
...
hf.dplot <- ggplot(SPLHF.long518, aes(x=Time, y=SPL_HF)) + geom_point() + theme( legend.position = "none") + 
  labs(y = "SPL 1-22 kHz (dB re 1 μ Pa)") + scale_x_discrete(breaks = c("00:00", "03:00", "06:00", "09:00", "12:00", "15:00", "18:00", "21:00")) +  
  theme_cowplot() + 
  ...
  geom_text(x=0, y=135, label="A)")

hf.dplot

#ACI

hfa.dplot <- ggplot(Snap.HFT518, aes(x=Time3, y =ACI_HF)) + geom_point() + theme( legend.position = "none") + 
  scale_x_discrete(breaks = c("00:00", "03:00", "06:00", "09:00", "12:00", "15:00", "18:00", "21:00")) + 
  scale_y_continuous(breaks = c(55000, 60000, 65000, 70000, 75000)) +
  theme_cowplot() + 
  theme(axis.title.x = element_blank()) + 
  labs(y = "ACI 1-22kHz") +
  theme(plot.title = element_text(hjust = 0.5, size = 15)) +
...
#ggsave(file = "../Figures/SPLHFDiurnal.pdf", width = 22, height = 7, hf.dplot)
```
...
mf.dplot.F <- mf.dplot.F + theme_cowplot() + labs(y = "SPL 0.16-1 kHz (dB re 1 μPa)")

mf.dplot.F <- mf.dplot.F + 
...
mf.dplot.F
#ggsave(file = "../Figures/SPLLFDiurnal.pdf", width = 22, height = 7, mf.dplot.F)

#ACI


mfa.dplot <- ggplot(ACI_MF_tot518, aes(x=Time, y =ACI_Midrange)) + geom_point() + theme(axis.text.x = element_text(angle = 90), legend.position = "none")

mfa.dplot.F <- mfa.dplot + scale_x_discrete(breaks = c("00:00", "03:00", "06:00", "09:00", "12:00", "15:00", "18:00", "21:00"))

mfa.dplot.F <- mfa.dplot.F + theme_cowplot() + labs(y = "ACI 0.16-1 kHz")

mfa.dplot.F <- mfa.dplot.F + 
  theme(axis.text.x = element_text(size = 12)) + 
  theme(axis.title.x = element_text (size = 15)) +
  theme(axis.text.y = element_text(size = 12)) + 
  theme(axis.title.y = element_text(size = 14))
...
##Combination Plot
```{r combination plot, echo = FALSE, fig.height = 6, fig.width = 10}

#adding in A,B,C labels

four.plot<- plot_grid(hf.dplot, hfa.dplot, mf.dplot.F, mfa.dplot.F,
          nrow = 2, ncol = 2,
          labels = c("A", "B", "C", "D"),
          align = "hv")
...
#ggsave(file = "../Figures/Fourplot_FINAL4.eps", width = 11, height = 7, four.plot)

#ggsave(file = "../Figures/Fourplot_FINAL4.pdf", width = 11, height = 7, four.plot)
...
hfa.dplot <- ggplot(Snap.HFT518, aes(x=Time3, y =ACI_HF)) + geom_point() + theme(axis.text.x = element_text(angle = 90)) 
hfa.dplot <- hfa.dplot + scale_x_discrete(breaks = c("03:00", "06:00", "09:00", "12:00", "15:00", "18:00", "21:00")) + theme_classic() + labs(y = "High Frequency ACI", x = "Time") +  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.title.y = element_blank())
hfa.dplot
#ggsave(file = "../Figures/hfa.dplot.pdf", width = 22, height = 7, hfa.dplot)



mfa.dplot <- ggplot(ACI_MF_tot518, aes(x=Time, y =ACI_Midrange)) + geom_point() 
mfa.dplot <- mfa.dplot + scale_x_discrete(breaks = c("03:00", "06:00", "09:00", "12:00", "15:00", "18:00", "21:00")) + theme_classic() + labs(y = "Low Frequency ACI", x = "Time")

mfa.dplot

#ggsave(file = "../Figures/mfa.dplot.pdf", width = 22, height = 7, mfa.dplot)
```
