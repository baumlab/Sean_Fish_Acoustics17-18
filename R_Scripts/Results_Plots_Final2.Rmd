...
simple.model <- lmer(SPL_HF ~ Snaps + (1|Site) + Year, data = Snap.HF)
...
simple.model1 <- lmer(SPL_HF ~ dn + (1|Site) + Year, data = Snap.HF)
...
simple.model2 <- lmer(SPL_HF ~ Snaps + dn + (1|Site) + Year, data = Snap.HF)
...
AICc(simple.model, simple.model1, simple.model2, simple.model4)
...
aci.hf <- lmer(ACI_HF ~ Snaps + (1|Site) + (1|Year), data = Snap.HF)
...
aci.hf1a <- lmer(ACI_HF ~ dn + (1|Site) + (1|Year), data = Snap.HF)
...
aci.hf2 <- lmer(ACI_HF ~ Snaps + dn + (1|Site) + (1|Year) , data = Snap.HF)
...
aci.hf4 <- lmer(ACI_HF ~ Snaps*dn + (1|Site) + (1|Year) , data = Snap.HF)
...
```{r model selection aci hf, echo = FALSE}

AICc(aci.hf, aci.hf1a, aci.hf2, aci.hf4)

```

The most parsimonious model here is aci.hf2, because it is the simplest model.

aci.hf2 includes: Snaps + dn + (1|Site) + (1|Year)

aci.hf4 includes: Snaps*dn + (1|Site) + (1|Year)


###Model Selection

```{r hf model selection, echo = FALSE}
AICc(aci.hf, aci.hf2, aci.hf4)

summary(aci.hf4)
```

When I compare all of these using AICc I find that the most parsimonious model is aci.hf4 which included Snaps + day/night + site and year as a nested random effect. So day/night must contribute to the model even if it is not significant.
...
simple1amf <- lmer(SPL_Midrange ~ Tot_Knocks + (1|Site)+(1|Year), data = AC.DF1Co)
...
Might be due to the random effects...


Total knocks are significant


```{r simple1b mf model, echo = FALSE}

simple1bmf <- lmer(SPL_Midrange ~ Num_L_calls + (1|Site)+(1|Year) , data = AC.DF1Co)
...
simple1cmf <- lmer(SPL_Midrange ~ Num_Herbivory + (1|Site)+(1|Year), data = AC.DF1Co)
...
simple2amf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + (1|Site)+(1|Year), data = AC.DF1Co)

summary(simple2amf)

```

Knocks is significant here, knocks has a tiny effect on MF SPL. Herbivory not significant.


```{r simple2b mf model, echo = FALSE}

simple2bmf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_L_calls + (1|Site)+(1|Year), data = AC.DF1Co)

summary(simple2bmf)

```

Total knocks is significant here too with a tiny effect size. Long calls not significant.

```{r simple2c mf model, echo = FALSE}

simple2cmf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + (1|Site)+(1|Year), data = AC.DF1Co)
...
simple3mf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + (1|Site) +(1|Year), data = AC.DF1Co)
...
Knocks and hour at 9 are significant.  Knocks has a slightly larger effect size than above and 9AM is 1.5 dB louder than 3PM.

###Adding 1 interaction term

```{r simple4 mf model, echo = FALSE}

simple4mf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + Tot_Knocks:Hour + (1|Site)+(1|Year), data = AC.DF1Co)

summary(simple4mf)

```

With the interaction here we see that knocks at 9AM is the only time with a significant positive growth. At 9AM each snap increases SPL by 0.031 dB.  Now 9AM is 2.3 dB louder than 3PM.  9PM is 1.25 dB louder than 3PM. Herbivory is also now significant although it has a very small effect size considering its low frequency.
...
simple6amf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + (1|Site)+(1|Year) + Tot_Knocks:Hour + Num_Herbivory:Hour, data = AC.DF1Co)

summary(simple6amf)
```

Adding this new interaction, really nothing changes. None of the levels of the interaction are significant. 


```{r simple6b mf model, echo = FALSE}

simple6bmf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + (1|Site/Year) + Tot_Knocks:Hour + Num_L_calls:Hour, data = AC.DF1Co)

summary(simple6bmf)
```

Here the effect size of total knocks at 9 AM is slightly smaller.  This is probably because the effect of long calls at 9 AM is significant, although long calls has a negative effect at 9 AM. 9AM is 1.7 dB louder than 3PM (the baseline) and 9PM is 1.12 dB higher than 3pm.  Total knocks at 3AM and 9PM are both significant but have effect sizes near zero.
...
simple7mf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + (1|Site)+(1|Year) + Tot_Knocks:Hour + Num_Herbivory:Hour + Num_L_calls:Hour, data = AC.DF1Co)
...
We don't see anything terribly new here. Now herbivory is significant and negative at 9 PM. Knocks and long calls are significant. Long calls at 9AM are still significant and negative and Knocksat 9AM are also stsill positive, back at around .03ish.

###Reduced Model

```{r simple reduced mf models, echo = FALSE}

reducemf <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + Tot_Knocks:Hour + (1|Site)+(1|Year), data = AC.DF1Co)


reducemf2 <- lmer(SPL_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + Tot_Knocks:Hour + Num_L_calls:Hour + (1|Site)+(1|Year), data = AC.DF1Co)


reducemf3 <- lmer(SPL_Midrange ~ Tot_Knocks + Hour + (1|Site)+(1|Year), data = AC.DF1Co)


reducemf4 <- lmer(SPL_Midrange ~ Tot_Knocks + Hour + Tot_Knocks:Hour + (1|Site)+(1|Year), data = AC.DF1Co)


#reducemf5 <- lmer(SPL_Midrange ~ Tot_Knocks + Hour + Tot_Knocks:Hour + Num_L_calls + (1|Site)+(1|Year), data = AC.DF1Co)
#model fails to converge

reducemf6 <- lmer(SPL_Midrange ~ Tot_Knocks + Hour + Tot_Knocks:Hour + Num_Herbivory + (1|Site)+(1|Year), data = AC.DF1Co)
...
```{r model selection mf spl, echo = FALSE}

AICc(simple1amf, simple1bmf, simple1cmf, simple2amf, simple2bmf, simple2cmf, simple3mf, simple4mf, simple6amf, simple6bmf, simple7mf, reducemf, reducemf2, reducemf3, reducemf4, reducemf6)

summary(reducemf6)
```

Best models are reducemf6, reducemf, and simple4mf

reducemf includes  Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + Tot_Knocks:Hour + (1|Site)+(1|Year)

reducemf6 includes: Knocks + Hour + Knocks*Hour + Num_Herbivory + (1|Site) + (1|Year)

simple4mf includes Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + Tot_Knocks:Hour + (1|Site)+(1|Year)

Therefore the most parsimonious model is reducemf6.
...
mf.aci1 <- lmer(ACI_Midrange ~ Tot_Knocks + (1|Site) + (1|Year), data = AC.DF1Co)
summary(mf.aci1)

```

Total knocks are significant and have a large effect size here...
...
mf.aci2a <- lmer(ACI_Midrange ~ Tot_Knocks + Num_Herbivory + (1|Site) + (1|Year), data = AC.DF1Co)
summary(mf.aci2a)

```

The addition of herbivory does nothing major, the effect size of knocks is even slightly higher.

```{r mf aci2b or Lon, echo = FALSE}

mf.aci2b <- lmer(ACI_Midrange ~ Tot_Knocks + Num_L_calls + (1|Site) + (1|Year), data = AC.DF1Co)
summary(mf.aci2b)

```

The addition of long calls also does nothing here.

###Step 3, Herbivory and Long Calls

```{r mf aci3, echo = FALSE}

mf.aci3 <- lmer(ACI_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + (1|Site) + (1|Year), data = AC.DF1Co)
summary(mf.aci3)
...
mf.aci4 <- lmer(ACI_Midrange ~ Tot_Knocks + Hour + (1|Site) + (1|Year), data = AC.DF1Co)
summary(mf.aci4)

#maximal model
mf.maxaci <- lmer(ACI_Midrange ~ (Tot_Knocks + Num_L_calls + Num_Herbivory)*Hour + (1|Site) + (1|Year), data = AC.DF1Co)

#reduced models
mf.acil <- lmer(ACI_Midrange ~ Num_L_calls + (1|Site) + (1|Year), data = AC.DF1Co)

mf.acih <- lmer(ACI_Midrange ~Num_Herbivory + (1|Site) + (1|Year), data = AC.DF1Co)

mf.acilh <- lmer(ACI_Midrange ~ Num_L_calls*Hour + (1|Site) + (1|Year), data = AC.DF1Co)

mf.acihh <- lmer(ACI_Midrange ~ Num_Herbivory*Hour + (1|Site) + (1|Year), data = AC.DF1Co)



```

Hour increases knock effect size but is not significant on its own.

###Main Effects Model

```{r mf aci6, echo = FALSE}

fit.mfaci6 <- lmer(ACI_Midrange ~ Tot_Knocks + Num_Herbivory + Num_L_calls + Hour + (1|Site) + (1|Year), data = AC.DF1Co)
summary(fit.mfaci6)


```

With all main effects, only knocks is significant still and has a high effect size.
...
fit.mfaci7 <- lmer(ACI_Midrange ~ Tot_Knocks*Hour + Num_Herbivory + Num_L_calls + (1|Site) + (1|Year), data = AC.DF1Co)
summary(fit.mfaci7)
...
AICc(mf.aci1, mf.aci2a, mf.aci2b, mf.aci3, mf.aci4, fit.mfaci6, fit.mfaci7, mf.maxaci, mf.acil, mf.acih, mf.acilh, mf.acihh)

summary(fit.mfaci6)
```

mf.maxaci is the best model.

mf.maxaci includes: (Tot_Knocks + Num_L_calls + Num_Herbivory)*Hour + (1|Site) + (1|Year)

but has a singular fit
